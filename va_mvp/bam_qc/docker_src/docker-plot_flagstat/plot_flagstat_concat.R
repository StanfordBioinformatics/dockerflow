...
make_boxplots <- function(data, plot_theme, out){
  ...
  ggplot(data, aes(series, total_reads)) +
    geom_boxplot() +
    plot_theme + 
    labs(title = "Bina ALS Genomes - QC Passed Reads", y = "Count", x = "Series")
  ggsave(file = paste(out, '.qc_reads_count.png', sep=''), width=14.4, height=8.1)
  
  # Percent mapped reads
  ggplot(data, aes(series, perc_mapped_reads)) +
    geom_boxplot() +
    plot_theme + 
    labs(title = "Bina ALS Genomes - Percent Mapped Reads", y = "Percent", x = "Series")
  ggsave(file = paste(out, '.perc_mapped_reads.png', sep=''), width=14.4, height=8.1)
  
  # Percent properly paired
  ggplot(data, aes(series, perc_properly_paired)) +
    geom_boxplot() +
    plot_theme + 
    labs(title = "Bina ALS Genomes - Percent Properly Paired", y = "Percent", x = "Series")
  ggsave(file = paste(out, '.perc_properly_paired.png', sep=''), width=14.4, height=8.1)
}
